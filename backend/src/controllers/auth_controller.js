...
import RecoveryToken from '../models/recovery_token_model.js';
...
export const changePassword = async (req, res) => {
  try {
    ...
    const paciente = await Paciente.findOne({ where: { id_paciente: token_row.paciente_id } });
    if (!paciente) {
      return res.status(404).json({
        code: -10,
    ...
    paciente.password = await bcrypt.hash(password, Number(process.env.BCRYPT_SALT));
    await paciente.save();
    //Elimino el token
    await RecoveryToken.destroy({
      where: {
        paciente_id: token_row.paciente_id
      }
    })
